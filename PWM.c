...
extern void Configura_Reg_PWM1(uint16_t freq)
{
    ...
    SYSCTL->RCGCGPIO |= (1<<1);
    SYSCTL->RCGCGPIO |= (1<<4); //Enable reloj de GPIO Puerto b pag 340 pin 4
    ...
    GPIOE->PCTL |= 0x00040000; //gen2 pin E4
  ...
  //  PWM0->_0_CTL = (1<<0);// Se activa el generador 2
    PWM0->ENABLE = (1<<0); //habilitar el bloque pa que pase, es el MnPWM2 en el pin2 Pag 1247
    PWM0->ENABLE = (1<<2);
    PWM0->ENABLE = (1<<4);
}
